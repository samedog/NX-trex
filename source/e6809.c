...
#define einline static __inline        /* <-- fixed: static __inline */
...
/* ---- register storage (unchanged from both) ---- */
static unsigned reg_x, reg_y, reg_u, reg_s, reg_pc;
static unsigned reg_a, reg_b, reg_dp, reg_cc;
static unsigned irq_status;

static unsigned *rptr_xyus[4] = { &reg_x, &reg_y, &reg_u, &reg_s };

/* ---- I/O function pointers (from e6809.c) ---- */
unsigned char (*e6809_read8)(unsigned address)            = 0;
void          (*e6809_write8)(unsigned address, unsigned char data) = 0;
void          (*e6809_instruction)(unsigned address)      = 0;

/* ---- trace flag ---- */
static int trace_cpu = 0;
void e6809_set_trace(int on) { trace_cpu = on; }
...
struct reg6809 *e6809_get_regs(void)
{
    ...
    r.pc = reg_pc;
    r.x = reg_x; r.y = reg_y; r.u = reg_u; r.s = reg_s;
    r.a = reg_a; r.b = reg_b; r.dp = reg_dp; r.cc = reg_cc;
    return &r;
}
...
{
	return (data & 0x80) ? (data | 0xffffff00u) : (data & 0xff);
}
...
einline unsigned exgtfr_read (unsigned reg)
{
	...
	default:
		data = 0xffff;
		if (trace_cpu)
			fprintf (stderr, "illegal exgtfr reg %.1x\n", reg);
		break;	
	}
...
unsigned e6809_sstep (unsigned irq_i, unsigned irq_f)
{
	...
	case 0x10:
		...
		default:
			if (trace_cpu)
				fprintf (stderr, "unknown page-1 op code: %.2x\n", op);
			break;
		}
	...
	case 0x11:
		...
		default:
			if (trace_cpu)
				fprintf (stderr, "unknown page-2 op code: %.2x\n", op);
			break;
		}
	...
	default:
		if (trace_cpu)
			fprintf (stderr, "unknown page-0 op code: %.2x\n", op);
		break;
	}
